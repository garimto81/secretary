...
def analyze_email_for_tasks(email_data: dict) -> dict:
    ...
    # 할일 감지 키워드
    action_keywords = [
        "확인해 주세요", "검토해 주세요", "보내주세요", "회신", "답변",
        "please review", "please check", "action required", "urgent",
        "마감", "deadline", "due date", "until", "by end of",
        "요청드립니다", "부탁드립니다", "처리해 주세요",
    ]
...
def format_output(tasks: list) -> str:
    ...
    if action_tasks:
        ...
        for task in sorted(action_tasks, key=lambda x: {"high": 0, "medium": 1, "low": 2}[x["priority"]]):
            priority_icon = {"high": "긴급", "medium": "보통", "low": "낮음"}[task["priority"]]
...
def main():
    ...
    parser.add_argument("--unread", action="store_true", default=True, help="미읽은 이메일만 분석")
